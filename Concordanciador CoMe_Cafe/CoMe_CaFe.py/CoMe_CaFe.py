...
while True:
    ...
    # Carregamento do arquivo - APENAS UMA VEZ
...
#introdução das ferramentas:
        ...
        if escolha_de_ferramentas not in ["File View", "word", "cluster", "Bigramas", "kwic", "1", "2", "3", "4"]:
            print("Ferramenta inválida. Tente novamente pressionando <Enter>.")
            continue

#codigo pro File View:
        if escolha_de_ferramentas == "File View" or escolha_de_ferramentas == "1":
            print("Você escolheu a ferramenta File View.")
            time.sleep(2)
            print("-" * 255)
...
#codigo pro Bigramas:
        elif escolha_de_ferramentas == "Bigramas" or escolha_de_ferramentas == "3":
            print("Você escolheu a ferramenta Bigramas.")
            time.sleep(2)
            print("-" * 255)
        ...
        # Exibir apenas bigramas com 2 ou mais ocorrências
            bigramas_frequentes = [(big, count) for big, count in freq.items() if count >= 2]
            bigramas_frequentes.sort(key=lambda x: x[1], reverse=True)
            if bigramas_frequentes:
                for bigrama, count in bigramas_frequentes:
                    print(f"{bigrama}: {count}")
            else:
                print("Nenhum bigrama com 2 ou mais ocorrências foi encontrado.")
...
# Loop para voltar ao menu principal ou encerrar o programa
        print("Digite <barra de espaço> para escolher outra ferramenta ou <esc> para encerrar o programa: ")
        while True:
            event = keyboard.read_event()
            if event.event_type == keyboard.KEY_DOWN:
                if event.name == 'space':
                    print("Voltando ao menu de ferramentas... Pressione <Enter> para continuar.")
                    input()
                    break
                elif event.name == 'esc':
